...
async function getTrackedProductIds() {
    ...
    if (error) {
        throw new Error(
            `Failed to load tracked products: ${error.message}`
        );
    }
...
async function main() {
    const PRODUCT_IDS = await getTrackedProductIds();

    console.log("Tracked products:", PRODUCT_IDS);

    if (PRODUCT_IDS.length === 0) {
        console.log("No tracked products found. Nothing to scrape.");
        return;
    }
